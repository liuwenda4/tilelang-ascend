...
def _build_kernel(
    ...
    def kernel_func(
        ...
        conv_state: T.Tensor(
            (symbol_cache_lines, symbol_state_len, symbol_dim), dtype_str
        ),
        ...
        with T.Kernel(num_batches * dim_num, is_npu=True) as (cid, vid):
            ...
            block_end = T.Select(
                block_offset + block_size > seqlen, seqlen, block_offset + block_size
            )
            ...
            has_initial = T.Select(
                batch_id == 0, initial_state_mode[0], initial_state_mode[1]
            )
...
def get_causal_conv1d_fn_pipeline_v15(
    weight_width, num_batches, dim, dtype_str="float16"
):
...
def causal_conv1d_fn_pipeline_v15(
    ...
    if dtype == torch.bfloat16:
        ...
        cs_copy = conv_states
    kernel = get_causal_conv1d_fn_pipeline_v15(
        weight.shape[0], cache_indices.size(0), x.size(1), kernel_dtype_str
    )
    output = kernel(
        x, weight, cs_copy.clone(), cu_seqlens, cache_indices, initial_state_mode
    )
    if dtype == torch.bfloat16:
...
def _run_test(dtype_str, dtype):
    tag = "float16" if dtype_str == "float16" else "bfloat16"
    print(f"\n{'=' * 60}\n[FN] Testing dtype: {tag}\n{'=' * 60}")
    ...
    print(
        f"baseDim={dim}, num_batches={num_batches}, dim_num={CORE_NUM}, Grid={grid_size}"
    )
    ...
    cs_ref = CS.cpu().clone()
    golden_output = causal_conv1d_fn_ref(
        X.cpu(), W.cpu(), cs_ref, CI.cpu(), QL.cpu(), IM.cpu()
    )
    ...
    t0 = time.time()
    for _ in range(3):
        _ = causal_conv1d_fn_pipeline_v15(X, W, CS.clone(), CI, QL, IM, dtype=dtype)
    torch.npu.synchronize()
    print(f"编译: {time.time() - t0:.2f}s")
    ...
    torch.npu.synchronize()
    print(f"耗时: {(time.time() - t0) / 10 * 1000:.3f}ms")
